...
function Row(props) {
  ...
  const removeEtapaResource = async (index) => {
    ...
    const etapaResource = await EtapaRemove({
      dono: user.email,
      nome: props.obj.nome,
    });
      ...
      <Table variant="light">
        <thead>
          <tr>
            <th></th>
            <th
              onClick={() => sortByHeader(0)}
              className={sorter === 0 ? "text-primary" : "text-dark"}
            >
              Nome
            </th>
            <th
              onClick={() => sortByHeader(1)}
              className={sorter === 1 ? "text-primary" : "text-dark"}
            >
              Duração
            </th>
