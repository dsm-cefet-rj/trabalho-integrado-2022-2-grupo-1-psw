...
function Cadeias() {
  ...
  useOnViewportChange({
    onChange: useCallback((viewport) => setView({x:viewport.x, y:viewport.y, zoom:viewport.zoom}), []),
  });
  ...
  function AddNode(node){
    ...
    setNodes(nds => nds.concat({
      ...
      targetPosition:'left',
      position:{
        x:((view.x * -1)/view.zoom) + ((document.getElementById("flow").offsetWidth/2)/view.zoom - 75),
        y:((view.y * -1)/view.zoom) + ((document.getElementById("flow").offsetHeight/3)/view.zoom)
      }
    }));
  }
  ...
  return (
    <div className="container mt-3 h-75">
      ...
      <div className="border rounded h-25 w-100 shadow mt-4 align-items-center d-flex p-5 overflow-auto">
        ...
        {
          etapasState.map(v => {
            return <CadeiaEtapa ordem={v.ordem} nome={v.nome} addHandler={AddNode}/>
          })
        }
        </div>
      </div>
